...
MonitorAgent.prototype.connect = function (port, host, cb) {
    ...
    this.socket.on('monitor', function (msg) {
        ...
        // command or response? 反正自己都是一个代理，总归是要交给consoleServer来执行的
        if (msg.command) {
        ...
        else
        {
            ...
            // request from master(使用monitorHandler来标记一个调用的发起方)
    ...
    // 由socket的error消息，进一步触发自身的error方法
 ...
 * @api public
 */
// 把自身的notify消息映射为socket层的‘monitor’消息
...
// 把自身的request消息映射为socket层的‘monitor’消息
